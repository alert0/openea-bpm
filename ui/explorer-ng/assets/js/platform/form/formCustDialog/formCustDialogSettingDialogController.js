...
app.controller('ctrl', [ '$scope', 'baseService', 'ArrayToolService', '$filter', function($scope, baseService, ArrayToolService, $filter) {
	...
	$scope.init = function() {
		...
		ToolsController.getEnum("org.openbpm.base.api.query.QueryOP").then(function(data) {
			$scope.$apply(function() {
				$scope.QueryOP = data;
			});
		});

		ToolsController.getEnum("org.openbpm.form.api.constant.FormCustDialogConditionFieldValueSource").then(function(data) {
			$scope.$apply(function() {
				$scope.FormCustDialogConditionFieldVALUESOURCE = data;
			});
		});
		
		ToolsController.getEnum("org.openbpm.base.api.constant.Direction").then(function(data) {
			$scope.$apply(function() {
